...
### 株式会社クリーマ (2024/11〜2026/09): インフラエンジニア

---

ECマーケットプレイスのインフラを担当した。
データベース基盤の移行、コストの最適化、bot対策、メール配信基盤の改善など、サービスを支える基盤全般を扱った。

#### Amazon EC2上のMySQLからAmazon Auroraへの移行 (2025/09〜2025/12)

自前で運用していたMySQLをAmazon Auroraへ移行した。移行後も、構成の見直しを継続的に行った。
...
サービスのメインデータベースを、Amazon EC2上のMySQLとして自前で運用していた。
冗長化やバックアップといった運用を全て自分たちで担う必要があり、少人数のチームには負担が大きかった。

また、移行を進める上で以下の障害があった。

- アプリケーションに、移行後の構成では期待どおりに動作しないデータベースアクセスが残っていた。
- レプリケーション構成を多用しており、環境の再構築に伴うダンプ/リストアが頻繁に発生していた。この処理はデータ量によっては5営業日かけても完了しないことがあった。

##### 解決策

【段階的な移行】

開発環境を一つずつ移行し、そこで手順の確立と問題の洗い出しを行った上で、本番環境の移行を実施した。
本番環境の移行はメンテナンスを設けて実施した。

【移行前提の是正】

移行後の構成では動作しないデータベースアクセスを、アプリケーションから排除した。
また、移行後の環境で主要な機能が期待どおりに動作するかを事前に検証した。

【ダンプ/リストア処理の並列化】

ダンプ/リストアを並列化するツールを自作した。

- 通常のテーブルは、1テーブルを1プロセスが担当する形で並列化する。
- 巨大なテーブルは、主キーの剰余でデータを分割し、1テーブルを複数プロセスで並列化する。
- スキーマの処理は直列、データの処理は並列とする。これはスキーマ操作時のメタデータロックの競合を避けるためである。

##### 技術選定

ダンプ/リストアの高速化には、まず既存の並列ダンプツールを検討した。
しかし、ダンプしたデータが重複する、リストア時にテーブルロックの取得に失敗する、といった問題が発生したため採用を見送った。

代わりに、mysqldumpを用いた並列化ツールを自作することとした。
mysqldumpは枯れており、ロックに関する挙動をオプションで細かく制御できる点を評価した。
実際、並列実行時のロック競合は、ロックに関するオプションを無効化することで回避できた。

特にインデックスの再構築を制御するオプションについては、公式ドキュメントを読み込むことで、
これが排他的なメタデータロックを要求すること、そのために分割したテーブルの並列リストアにおいて先に完了したプロセスが他のプロセスの挿入をブロックしてしまうこと、を特定した。

##### 成果

- 本番環境を含む全ての環境をAuroraに移行した。
- ダンプ/リストアの処理時間を短縮した。5営業日かけても完了しなかった処理が、1営業日で完了するようになった。
   - 移行時に限らず、移行後も残るレプリケーション運用の作業時間を恒常的に削減できた。
- 移行後も、シングルAZ構成への変更、バイナリログの無効化、I/O最適化の適用など構成の見直しを継続し、コストを削減した。

##### 工夫した点

自作したツールには、リストアを行う前にダンプが可能かを検証する処理と、処理後にデータの整合性を確認する仕組みを組み込んだ。
一人で長時間の処理を回す以上、失敗を早期に検知できること、結果を機械的に確認できることが必要だと考えた。

</details>

---

#### AWSコストの可視化と継続的な削減 (2025/11〜2026/08)

コストの追跡ができていない状態を解消した上で、削減施策の立案と実施を継続的に行った。
...
現状の調査から施策の立案、実施、効果測定まで一貫して担当した。
...
コスト配分タグが設定されていないリソースが多く、コストがどこで発生しているかを追跡できない状態だった。
そのため、予算の策定も、削減施策の効果測定も精度を欠いていた。

##### 解決策

【可視化】

タグで追跡できていないリソースを洗い出し、環境ごとにタグを付与した。
この過程で、どこからも使われていないリソースが相当数残っていることが判明したため、合わせて削除した。

【削減施策の立案と実施】

可視化により判明したコストの内訳を元に、影響の大きいものから施策を立案・実施した。

- データベース: 冗長構成の見直し、不要なバイナリログの無効化、料金体系の切り替え、不要なスナップショットの削除
- 購入プラン: 割引購入プランの適用状況の調査と見直し
- 開発環境: 不要なリソースの統合・削除、使用しない時間帯の自動停止、保持データの削減
- 監視・ログ: 記録頻度や保存方式の見直し
- ライセンス: 開発用ツールの代替手段への移行

##### 技術選定

料金体系の切り替えについては、切り替えるべきかどうかの判断式を作成した上で適用した。
サービスの料金体系は、利用パターンによって有利不利が入れ替わる。個別の勘に頼らず、
実測値を入力すれば判断できる形にすることで、他のリソースにも同じ基準で適用できるようにした。

##### 成果

- コスト配分タグの整備により、予算策定と効果測定の前提となるコストの可視性を確保した。
- 上記の施策により、開発環境のコストを約6,000ドル/月 削減した。
   - データベースの移行に伴う構成の見直しによる削減分を含む。

##### 工夫した点

削減の判断は、公式の料金ドキュメントと実際の使用量の両方を確認した上で行った。
特に、料金体系の切り替えや割引購入プランのように後戻りしづらい変更については、
事前に費用の比較を文書化し、判断の根拠を残すようにした。

</details>

---

#### 分散スクレイピングbotへの対策 (2026/07〜)

分散スクレイピングbotによるアクセスに対し、WAFでの対策を設計・実施した。
...
分散スクレイピングbotによるアクセスが発生していた。
JavaScriptを実行するヘッドレスブラウザによるアクセスであり、単純なUser-Agentやレート制限では検知・遮断が難しかった。

また、サービスによってWAFの提供元が異なっていたため、両者で対策の内容を揃える必要があった。

##### 解決策

WAFのアクセスログを分析基盤に取り込み、AS単位でアクセス状況を集計した。
その結果を元に、対象とするASを決定し、当該ASからのアクセスにチャレンジを課すルールを設計した。

チャレンジの対象は、トップレベルのドキュメントへのアクセスに限定した。
非同期通信やサブリソースへのアクセスにチャレンジを課すと、正規の利用者の画面が破綻するためである。

##### 技術選定

チャレンジページを両サービスで統一したカスタムページにするかを検討したが、見送った。

公式ドキュメントを一次情報として調査した結果、チャレンジページのカスタマイズ性が両者で大きく異なることが分かった。
片方はエッジ側の設定のみでカスタマイズできる一方、もう片方はSDKの組み込みとアプリケーションの改修を必要とする。
この非対称性を踏まえると、統一のために支払うコストが、得られるブランド上の利点に見合わないと判断した。

##### 成果

- アクセス状況の分析に基づき、対策の対象と方式を決定した。
- 両サービスで同じ考え方の対策を、それぞれのWAFの制約に合わせた形で適用する方針を確立した。

##### 工夫した点

無音のチャレンジは、JavaScriptを実行するbotに対しては効果が限定的である。
そのため、チャレンジ単体で解決するとは考えず、ASによる絞り込みやレート制限と組み合わせることを前提に設計した。
...
送信ドメインの認証を整備し、あわせて配信の異常を検知できる状態を整えた。
...
1つは、なりすましメールへの対策が不十分だったことである。
メールを送信しないドメインについては、そもそも認証に関するレコードが設定されていなかった。

もう1つは、配信状況を把握する仕組みがなかったことである。
メールの配信基盤はPostfixによるメールサーバ群として自前で構築・運用しており、
マネージドの配信サービスであれば標準で提供される配信状況の指標が存在しない。
そのため、バウンスの急増や送信キューの滞留が発生しても、それを早期に検知できなかった。

##### 解決策

【送信ドメインの認証】

メールを送信するか否かによらず、全てのドメインにMX、SPF、DKIM、DMARCのレコードを設定する方針を策定し、合意を得た。
送信しないドメインであっても、なりすましの踏み台にされ得るためである。

方針に沿って、サブドメインを含めたゾーンの整理と認証レコードの設定を進めた。
あわせて、外部のメール送信サービスを利用しているケースを棚卸しし、それらについても認証の対象に含めた。

【配信状況の監視】

以下の2つを監視の対象とした。

- バウンスの発生数: 通常時の水準を調査した上で、そこからの乖離を検知する
- 送信キューの長さ: 配信の滞留を検知する

メールサーバは自前で運用しているため、これらの指標はそのままでは存在しない。
そのため、Lambdaでメールサーバ上の情報を集計し、カスタムメトリクスとして発行した上で、
それに対してアラームを設定する構成とした。

##### 技術選定

認証レコードの設定は、DNSの管理そのものをコード化した上で行った。
ドメインとサブドメインの数が多く、手作業では設定の漏れや不統一が生じるためである。

監視については、メールサーバ自体に監視の仕組みを組み込む選択肢もあったが、採らなかった。
サーバのプロビジョニングを複雑にすることになるためである。

Lambdaを用いて外部から監視する構成であれば、メールサーバ自体には手を入れずに済む。
監視の追加や変更も、メールサーバの構成とは独立して行える。

##### 成果

- 送信の有無によらず認証レコードを設定するという方針を定め、適用を進めた。
   - 多くのドメインでは、DMARCのポリシーを`p=reject`まで引き上げている。全てのドメインへの適用は継続中である。
- バウンスの急増と送信キューの滞留を検知する監視を、実際の発生状況の分析に基づいて設計・実装した。

##### 工夫した点

なりすまし対策は、レコードを設定するだけでは完結せず、
正規のメールが認証を通過できているかの確認を伴う。
そのため、認証の仕組みそのものと、送信元の整合性に関する仕様を確認した上で、
既存の送信経路が方針に適合するかを一つずつ検証した。

</details>

---

### ブルーモ証券株式会社 (2023/04〜2024/09): インフラエンジニア

---

インフラを中心に、人手が足りない際にバックエンドサーバ(Golang)、BFF(Rails)の機能追加・改修等も行った。

#### Kubernetesクラスタ上でのサービス構築・運用 (2023/04〜2024/09)
...
サービスの一般公開を控えていたが、インフラはほぼ整備されていない状態だった。
公開に向けて、本番運用に耐えるインフラの構築と、その運用プロセスの確立が必要だった。

また、インフラを担当するエンジニアが自分一人であったため、構築するだけでなく、構築後も一人で運用を継続できる状態にすることが求められた。

##### 解決策

以下の観点でサービス基盤を構築し、合わせて運用プロセスを整備した。

【複数環境のマニフェストの整備】

Kustomizeを用いて、共通の定義をbase、環境毎の差分をoverlayとして管理した。
環境間の差分が明示されることで、環境毎の設定の食い違いを防ぎ、環境の追加も容易にした。

【CI/CDパイプラインによるデプロイの半自動化】

Cloud Buildを用いて、コンテナイメージのビルドからマニフェストの適用までをパイプライン化した。
デプロイするかどうかの判断は人が行い、それ以降の手順は自動化する半自動の形とした。

【サービスから利用するGCPリソースのコード化】

Cloud SQL、Cloud Storageなど、サービスから利用するGCPリソースをTerraformでコード化した。
変更前にterraform planで差分を確認できる状態を作り、一人での運用における事故を防いだ。

【監視設計】

- 構造化ログの導入: アプリケーションのログを構造化して出力し、Cloud Logging上で検索・集計できるようにした。
- ゴールデンシグナルの可視化: レイテンシ、トラフィック、エラー、サチュレーションをCloud Monitoringのダッシュボードに集約し、サービスの状態を一目で把握できるようにした。
- トレースの導入: OpenTelemetryでアプリケーションを計装し、Cloud Traceにトレースを送信した。

【負荷試験と合わせたリソースサイジング】

Locustで負荷試験を実施し、要求されるパフォーマンスを満たすリソース量を確認した上で、Kubernetesのリソース設定とGKEのノード構成を決定した。

##### 技術選定

作業者が自身のみという状況において、可能な限りGCPのマネージドサービスに寄せる判断を取った。
運用の担い手が増える見込みが薄い以上、自前で構築・運用するコンポーネントを増やさない事を優先した。

他方、コストを抑えつつトレースを導入するためにOpenTelemetryを採用する等、必要であるならば新しい技術も積極的に取り入れた。
...
アクセス急増によるサービスダウンの際は、まずリソース増強による復旧を優先し、その後に負荷試験を通してサイジングを見直した。
応急対応と恒久対応を切り分ける事で、サービスの可用性を確保しつつ、勘に頼らないサイジングに落とし込む事ができた。
